%macro sas_sec_ripemd320(file, _rmd320_);
    %local num;
    %let num=510;

    data _null_;
        if filename('_rmd320_', "&file.") then
            do;
                put 'error: 文件分配错误!';
                return;
            end;
        ...
        rmd320=put(left(reverse(input(put(ha, hex8.), $hex8.))), $hex8.) 
            ...
            ||put(left(reverse(input(put(hj, hex8.), $hex8.))), $hex8.) 
            ||put(left(reverse(input(put(hk, hex8.), $hex8.))), $hex8.);

        if compress("&_rmd320_.")='' then
            do;
                put '文件' "&file." '字节数为' n 'bytes.';
                put rmd320=;
            end;
        else if symexist("&_rmd320_.") then
            call symput("&_rmd320_.", rmd320);
        else
            do;
                rc=filename('rmd320__', "&file..rmd320");
                oid=fopen('rmd320__', 'o', 80, 'b');
                rc=fput(oid, rmd320);
                rc=fwrite(oid);
                rc=fclose(oid);
                rc=filename('rmd320__');
            end;
    run;
