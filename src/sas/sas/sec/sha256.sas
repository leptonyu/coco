%macro sas_sec_sha256(file, _sha256_);
    %local num;
    %let num=510;

    data _null_;
        if filename('_sha256_', "&file.") then
            do;
                put 'error: 文件分配错误!';
                return;
            end;
        ...
        sha256=put(ha, hex8.)||put(hb, hex8.)||put(hc, hex8.)||put(hd, 
            hex8.)||put(he, hex8.)||put(hf, hex8.)||put(hg, hex8.)||put(hh, 
            hex8.);

        if compress("&_sha256_.")='' then
            do;
                put '文件' "&file." '字节数为' n 'bytes.';
                put sha256=;
            end;
        else if symexist("&_sha256_.") then
            call symput("&_sha256_.", sha256);
        else
            do;
                rc=filename('sha256__', "&file..sha256");
                oid=fopen('sha256__', 'o', 64, 'b');
                rc=fput(oid, sha256);
                rc=fwrite(oid);
                rc=fclose(oid);
                rc=filename('sha256__');
            end;
    run;
